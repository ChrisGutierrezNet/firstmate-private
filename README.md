...
| `/simplify-changes` | Review a named set of recent changes from four narrow perspectives - reuse, quality, efficiency, and altitude - then report the findings, applying only the clearly safe ones through the normal crewmate delivery path. Opt-in only: it never runs on its own after an edit, a task, or a pipeline stage |
