...
Cross-home reads validate the seeded identity and operational-directory boundaries, bound each home by time and output size and the whole pass by a cross-home budget, and classify unavailable, malformed, or inconsistent structured state as unknown rather than reviving a parent event as current work.
The bounds separate transport from content, because those two failures need opposite handling.
A home that never answered - timed out, exhausted the cross-home budget, exited non-zero, exceeded the byte limit, returned malformed or stale output, or has an unusable path - is genuinely unread, so it drops to the strict empty parent-event fallback with no child-derived surfaces.
