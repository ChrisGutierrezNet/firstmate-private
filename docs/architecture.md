...
Cross-home reads validate the seeded identity and operational-directory boundaries, bound each home by time and output size and the structured-home summary reads across all homes by a cross-home budget, and classify unavailable, malformed, or inconsistent structured state as unknown rather than reviving a parent event as current work.
The per-home parent-activity and terminal-evidence bounds sit outside that budget, so the aggregate ceiling is the budget plus those supplemental per-home bounds across homes, which is still far below the previous per-home-bound times home-count worst case.
The bounds separate transport from content, because those two failures need opposite handling.
A home that never answered - timed out against its own per-home bound, had its read cut short or never reached by the cross-home budget, exited non-zero, exceeded the byte limit, returned malformed or stale output, or has an unusable path - is genuinely unread, so it drops to the strict empty parent-event fallback with no child-derived surfaces.
A home that answered with a schema-valid, in-bounds, same-observation summary was read, so a contradiction inside its own inventory makes only the classification unknown: it keeps structured-home provenance with partial trust, names the contradiction as its reason, and retains the independently derived working children, decisions, holds, queued and landed records, endpoint identities, and counts.
Treating a content contradiction as an unread home would discard a whole home's readable current work, including captain-actionable holds, over one bookkeeping mismatch.
Every payload that grows with a home's inventory travels to `jq` over stdin rather than as a process argument, because `execve` caps a single argument at 128 KiB on Linux and the whole argument block on macOS; passing a growing inventory as an argument made a large healthy home fail the read outright and report as unavailable.
...
Later milestone wakes use `bin/fm-x-followup.sh` to post up to three public-safe follow-ups through the relay's `connector/followup` endpoint, ending with a `--final` one for ordinary X-linked work. A typed promised-final commitment owns its terminal reply through `bin/fm-public-followup.sh`; after its receipt is validated, `bin/fm-x-followup.sh --clear <task-id>` removes any legacy link without posting another reply.
